...
const VerifyForm = (props) => {
    ...
    const submitHandler = (event) => {
        ...
        const enteredCardNumberIsValid = !isEmpty(enteredCardNumber) && isCreditCardLongEnough(enteredCardNumber);
        const enteredExpirationDateIsValid = !isEmpty(enteredExpirationDate);
        const enteredCVCIsValid = !isEmpty(enteredCVC) && isCvcLongEnough(enteredCVC);
        ...
        const formIsValid = enteredCardNumberIsValid && enteredExpirationDateIsValid && enteredCVCIsValid && enteredUserNameIsValid 
        ...
        const cardNumberControlClasses = `${styles.control} ${formInputValidity.cardNumber ? "" : styles.invalid}`;
        const expirationDateControlClasses = `${styles.control} ${formInputValidity.expirationDate ? "" : styles.invalid}`
        const cvcControlClasses = `${styles.control} ${formInputValidity.cvc ? "" : styles.invalid}`;
        const userNameControlClasses = `${styles.control} ${formInputValidity.userName ? "" : styles.invalid}`        
